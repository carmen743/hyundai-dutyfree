...
// 국적/외형 출시 빈도는 프롬프트가 아니라 서버 코드에서 명시적 퍼센트로 관리한다.
// 베트남/필리핀 등 동남아시아계는 제외하고, 기존 비율을 남은 그룹에 재분배한다.
const ETHNICITY_WEIGHTS = [
  { id: 'east-asian', percent: 35 },
  { id: 'european', percent: 39 },
  { id: 'mixed', percent: 12 },
  { id: 'south-asian', percent: 6 },
  { id: 'latine', percent: 4 },
  { id: 'black-african', percent: 2 },
  { id: 'middle-eastern', percent: 2 },
];
const ETHNICITY_BY_ID = new Map(ETHNICITY_GROUPS.map((group) => [group.id, group]));
...
function getWeightedEthnicityCandidates(previousEthnicity = '') {
  return ETHNICITY_WEIGHTS
    .map(({ id, percent }) => ({ ...ETHNICITY_BY_ID.get(id), weight: percent }))
    .filter((group) => group.id && group.weight > 0 && group.id !== previousEthnicity && group.ko !== previousEthnicity);
}

function pickWeightedBySeed(weightedItems, seed, salt) {
  const candidates = weightedItems.filter((item) => item && item.weight > 0);
  const total = candidates.reduce((sum, item) => sum + item.weight, 0);
  if (!total) return null;
  let cursor = (hashString(`${seed}|${salt}`) / 0x100000000) * total;
...
export function buildGenerationContext({ name, birth, destination, gender, resultSeed = '' }, options = {}) {
  ...
  const ethnicityCandidates = getWeightedEthnicityCandidates(previousEthnicity);
  return {
    ethnicity: pickWeightedBySeed(ethnicityCandidates, seed, 'ethnicity') || pickWeightedBySeed(getWeightedEthnicityCandidates(), seed, 'ethnicity'),
...
imagePrompt를 제외한 모든 사용자 노출 텍스트 필드는 한국어로 작성한다. name은 반드시 한글 음역만 사용하고, 영어/라틴 알파벳/원어 문자를 섞지 않는다.
스키마는 아래 키만 사용한다.
{
  "tagline": string,
  "name": string,
...
- name: 사용자가 입력한 이름을 절대 사용하지 않는다. 여행지 국적과 내부 인종/민족 배정에 어울리는 현지 이름을 반드시 한글 음역으로만 쓴다. 라틴 알파벳·영문 이름·원어 표기 금지. 예: Adrien Laurent → 아드리앙 로랑, Sofia Rossi → 소피아 로시, Kenji Sato → 켄지 사토.
- nationality: 여행지에 맞는 국적을 한국어로 쓴다. 예: 프랑스, 이탈리아, 일본, 미국.
- job: 여행지 분위기에 어울리는 직업을 한국어로 쓴다. 특정 브랜드명 금지.
- personality: 성격 태그 정확히 3개. 각 태그는 짧게.
- style: "OOO형" 형식의 스타일 태그 1개.
- quote: 연애 시뮬레이션 캐릭터 대사처럼 감정이 실린 한마디. 사용자 이름은 호칭으로만 사용 가능. 단, 19금·섹슈얼·선정적 뉘앙스는 절대 금지하고 청량하고 다정한 설렘만 허용한다.
- story: 정확히 7개 문단 배열. 씬(Scene) 제목 없이 소설체 연속 문단. 청량하고 귀여운 설렘 중심으로 쓴다. 문단은 서로 자연스럽게 이어져야 한다. 3번째 문단은 감정이 전환되는 클라이맥스 문단으로 쓰되, 19금·섹슈얼·선정적 상황이나 신체 접촉 묘사는 절대 쓰지 않는다.
...
- 모든 사용자 노출 텍스트(tagline, name, nationality, job, personality, style, quote, story)에는 19금스럽거나 섹슈얼한 표현, 선정적 분위기, 노출·신체 부위 강조, 유혹/도발/관능/야릇함, 키스·스킨십·침대 장면을 절대 포함하지 않는다.
- style과 personality에는 "섹시형", "관능형", "도발형"처럼 성적 매력을 전면에 둔 유형을 절대 쓰지 않는다.
- 인연 표현은 청량함, 다정함, 귀여운 설렘, 여행의 우연, 취향 공유 중심으로만 작성한다.
...
- 모든 인물은 respectful, photogenic, pleasant, natural actor/influencer 느낌으로 만든다. 19금스럽거나 섹슈얼한 분위기, 선정적 포즈, 신체 부위 강조, 노출이 많은 분위기 금지.
...
const IMAGE_SAFETY_SUFFIX = 'PG-rated family-friendly portrait with an innocent friendly travel mood. Respectful non-sexualized portrait. Modest stylish travel outfit. No cleavage, no lingerie, no swimwear, no deep neckline, no bare skin emphasis, no sexualized pose, no erotic styling, no seductive expression, no intimate physical contact, no kissing, no bedroom setting, no text, no UI, no card layout, no graphics, no subtitle, no watermark, no logo, no illustration, no cartoon, no duplicated faces, no distorted hands, fictional person not resembling any real person.';

export function enforceImagePromptSafety(prompt, { gender, ethnicityInstruction } = {}) {
  const base = String(prompt || '')
    .replace(/19금|섹슈얼|섹시|관능|도발|야릇|므흣|선정|노출|가슴|란제리|비키니|수영복/gi, 'family-friendly')
    .replace(/\b(sexy|sex|sexual|seductive|sensual|erotic|provocative|suggestive|adult|nsfw|intimate|kissing?|cleavage|lingerie|bikini|swimsuit|revealing|deep neckline|bedroom|nude|naked)\b/gi, 'family-friendly modestly dressed')
...
export function buildImagePrompt({ destination, gender, generationContext }) {
  ...
  const portraitDirection = `appealing photogenic fictional ${genderEn} in their late 20s, respectful, family-friendly, non-sexualized, natural actor or influencer presence, ${imageVibe}`;
